...
def get_active_nodes():
    try:
        ...
        print(f"[GET ACTIVE NODES ERROR] {e}")
        return {}


def register_node(chat_id, name):
    try:
        existing_jobs = firebase_get(f"nodes/{chat_id}/jobs")
        jobs = existing_jobs if isinstance(existing_jobs, int) else 0
        node_data = {
            "name": name,
            "chat_id": chat_id,
    ...
    try:
        ...
        if text == "/start":
            ...
            if existing and isinstance(existing, dict):
                ...
                if isinstance(last_seen, (int, float)) and now - last_seen < 180:
                    send(
                        message["chat"]["id"],
            ...
            if node and isinstance(node, dict):
                now = datetime.now(timezone.utc).timestamp()
                last_seen = node.get("last_seen", 0)
                is_active = isinstance(last_seen, (int, float)) and now - last_seen < 180
                send(
                    message["chat"]["id"],
